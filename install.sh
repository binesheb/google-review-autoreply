...
# Validate archive member names before extraction. GitHub source archives should
# contain one repository-root directory only; reject absolute paths or traversal
# entries so a compromised archive cannot write outside TMP_DIR during extraction.
if tar -tzf "$ARCHIVE" | awk '
  BEGIN { bad=0 }
  {
    path=$0
    if (path ~ /^\// || path ~ /(^|\/)\.\.\// || path == ".." || path ~ /^\.\.\//) bad=1
    if (path !~ /^[^\/]+\//) bad=1
  }
  END { exit bad }
'; then
  ...
  die "Downloaded archive contains unsafe or unexpected paths. Existing installation was not modified."
fi
