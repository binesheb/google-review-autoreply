...
from app.models import Review, Location, Approval, AuditLog, OwnerReply, ReviewVersion
...
def ingest_review(payload: ReviewIn, db: Session = Depends(get_db), actor: str = Depends(require_admin)):
    ...
    if not review:
        ...
        db.add(review)
        db.flush()
        db.add(ReviewVersion(review_id=review.id, version=1, rating=review.rating, comment=review.comment, has_owner_reply=review.has_owner_reply))
    else:
        changed = review.rating != payload.rating or review.comment != payload.comment or review.has_owner_reply != payload.has_owner_reply
        if changed:
            latest = db.scalar(select(ReviewVersion).where(ReviewVersion.review_id == review.id).order_by(ReviewVersion.version.desc()))
    ...
    db.add(AuditLog(action="review_ingested", actor=actor, target_type="review", target_id=payload.source_review_id, detail=payload.source_name))
    db.commit()
    db.refresh(review)
...
def approve(review_id: int, payload: ActionRequest, db: Session = Depends(get_db), actor: str = Depends(require_admin)):
    ...
    db.add(Approval(review_id=review.id, draft_id=latest.id, action="approve", actor=actor, comment=payload.comment))
    review.status = "approved"
    db.add(AuditLog(action="review_approved", actor=actor, target_type="review", target_id=str(review.id), detail=payload.comment))
    db.commit()
    return {"status": "approved", "review_id": review.id}


@router.post("/{review_id}/publish")
def publish(review_id: int, payload: ActionRequest, db: Session = Depends(get_db), actor: str = Depends(require_admin)):
    review = _get_review(db, review_id)
    ...
    if not approved and not (settings.auto_publish_enabled and latest.auto_eligible and not settings.automation_paused):
        raise HTTPException(403, "Approval is required before publishing")
    result = GoogleBusinessProfileClient(settings.google_access_token).update_reply(review.source_name, latest.response_text)
    review.has_owner_reply = True
    review.status = "published"
    db.add(OwnerReply(review_id=review.id, response_text=latest.response_text, actor=actor))
    db.add(AuditLog(action="owner_reply_published", actor=actor, target_type="review", target_id=str(review.id), detail=payload.comment))
    db.commit()
