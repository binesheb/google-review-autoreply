...
def login(payload: LoginRequest, response: Response):
    if payload.username != settings.admin_username or not verify_password(payload.password, settings.admin_password_hash):
        return Response(content='{"detail":"Invalid credentials"}', media_type="application/json", status_code=401)
    response.set_cookie("rip_user", payload.username, httponly=True, samesite="lax", secure=False, max_age=8 * 3600)
