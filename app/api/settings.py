...
def get_settings(db: Session = Depends(get_db), _: str = Depends(require_admin)):
    ...
    return {
        ...
        "automation_paused": stored.get("automation_paused", str(settings.automation_paused).lower()) == "true",
...
@router.put("/{key}")
def update_setting(key: str, payload: SettingUpdate, db: Session = Depends(get_db), actor: str = Depends(require_admin)):
    ...
    db.add(AuditLog(action="setting_changed", actor=actor, target_type="setting", target_id=key, detail=payload.value))
    db.commit()
