...
from app.models import KnowledgeItem, AuditLog, Organization
...
@router.post("")
def create_knowledge(payload: KnowledgeIn, db: Session = Depends(get_db), actor: str = Depends(require_admin)):
    ...
    db.add(item)
    db.flush()
    db.add(AuditLog(action="knowledge_created", actor=actor, target_type="knowledge", target_id=str(item.id), detail=payload.title))
    db.commit()
    db.refresh(item)
    return item
...
@router.put("/{item_id}")
def update_knowledge(item_id: int, payload: KnowledgeIn, db: Session = Depends(get_db), actor: str = Depends(require_admin)):
    ...
    db.add(AuditLog(action="knowledge_updated", actor=actor, target_type="knowledge", target_id=str(item.id), detail=payload.title))
    db.commit()
    db.refresh(item)
    return item
